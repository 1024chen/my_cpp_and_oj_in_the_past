...
//普里姆算法
//源点，closeVex[v]存放与v最近的顶点，lowWeight[v]存放(closeVex[v],v)权值
Status Prim(int k, int *closeVex, ElemType *lowWeight, LGraph *lg)
{
    ENode *p;
    int i, j;
    ElemType min;

    int *isMark = (int *)malloc(sizeof(int) * lg->n); //动态生成一维isMark数组，标识是否加入生成树
    closeVex = (int *)malloc(sizeof(int) * lg->n);
    lowWeight = (ElemType *)malloc(sizeof(ElemType) * lg->n);

    if (k < 0 || k > lg->n - 1)
        return ERROR;
    for (int i = 0; i < lg->n; i++) //三个辅助数组初始化
    {
        closeVex[i] = -1;
        lowWeight[i] = INFINTY;
        isMark[i] = 0;
    ...
    for (i = 1; i < lg->n; i++) //选择其余n-1条边加入生成树
    {
        ...
        {
            j = p->adjVex;
            if ((!isMark[j]) && (lowWeight[j] > p->w)) //如果此邻接点尚未加入并且比之前求出来的最小权值还要小
            {
                closeVex[j] = k;     //更新该点的最近顶点
                lowWeight[j] = p->w; //和最小权值
            }
        }

        min = INFINTY;
        for (j = 0; j < lg->n; j++) //寻找生成树外顶点中，具有最小lowWeigh值的顶点k
        {
...
int main()
{
    ...
    int *closeVex, *lowWeight;
    Prim(0, closeVex, lowWeight, &lg);
    DestoryLGraph(&lg);
    return 0;
}